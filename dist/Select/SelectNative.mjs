...
const SelectNative = forwardRef(({ className, defaultValue, disabled, onChange, options: optionsProp, readOnly, style, value: valueProp, variant, width, ...otherProps }, ref) => {
  ...
  return React.createElement(
    ...
    React.createElement(
      StyledInner,
      null,
      React.createElement(StyledNativeSelect, { ...otherProps, disabled, onChange: isEnabled ? handleChange : noOp, ref, value }, options.map((option, index) => {
        var _a;
        return React.createElement("option", { key: `${option.value}-${index}`, value: option.value }, (_a = option.label) !== null && _a !== void 0 ? _a : option.value);
      })),
      DropdownButton
    )
  );
